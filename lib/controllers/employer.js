...
const getOrderList = async(ctx, next) => {
    ...
    try{
        let {employerId} = ctx.request.body;
        result.data = await query(`select *,(select count(*) from log where log.Project_ID = project.Project_ID and log.IsRead_Employer = 0) as newsNumber from project where Employer_ID = '${employerId}' order by Riqi DESC`);
        result.success = true;
    }catch(e){
        result.success = false;
        result.msg = e.message;
...
module.exports = {
    ...
    'POST /employer/getOrderList': getOrderList
}