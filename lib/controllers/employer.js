...
const getOrderList = async(ctx, next) => {
    ...
    try{
        let {employerId} = ctx.request.body;
        result.data = await query(`select Project_ID,Name,State,Employer_ID,Employee_ID,Length_real,Wage_real from project where Employer_ID = '${employerId}' order by Riqi DESC`);
        result.success = true;
    }catch(e){
        result.success = false;
        result.msg = e.message;
