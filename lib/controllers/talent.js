...
const getOrderList = async(ctx, next) => {
    ...
    try{
        let {employeeId} = ctx.request.body;
        result.data = await query(`select *,(select count(*) from log where log.Project_ID = project.Project_ID and log.IsRead_Employee = 0) as newsNumber from project where Employee_ID = '${employeeId}' order by Riqi DESC`);
        result.success = true;
    }catch(e){
        result.success = false;
        result.msg = e.message;
