...
const getEmployeeInfo = async(ctx, next) => {
    ...
    try{
        let {employeeId} = ctx.request.body;
        let requiredFields = 'Employee_ID,Name,Job_company,Job_position,Job_experience,Rating,Skills,Projects,Wage,State,Ordered_number';
...
const getOrderList = async(ctx, next) => {
    ...
    try{
        let {employeeId} = ctx.request.body;
        result.data = await query(`select Project_ID,Name,State,Employer_ID,Employee_ID from project where Employee_ID = ${employeeId} order by Riqi DESC`);
        result.success = true;
    }catch(e){
        result.success = false;
        result.msg = e.message;
