...
const addCheckPayTask = (Project_ID,type)=>{
    ...
    cache.put('CheckPay_'+Project_ID, '', config.orderPayTimeout*60*60*1000,async()=>{
        let searchResult = await query(`select a.Amount_paid,a.Employee_ID,b.Name from project a left join employee b on a.Employee_ID = b.Employee_ID where a.Project_ID = ${Project_ID}`);
        if(searchResult && searchResult.length > 0){
            let moneyPaid = searchResult[0].Amount_paid;
            if(moneyPaid === null || moneyPaid == 0){//删除订单
            ...
            if(data.type === 'publish'){
                ...
                if(employeeInfo && employeeInfo.length > 0){
                    if(employeeInfo[0].State === 0){
                        let addSql = `insert into project (Name,Wage,Wage_real,Length,Length_real,Employer_ID,Employee_ID,State,Riqi) values ('${data.Name}',${data.Wage},${data.Wage},${data.Length},${data.Length},'${data.Employer_ID}','${data.Employee_ID}',2,'${moment().format("YYYY-MM-DD HH:mm:ss")}')`;
...
const applyForRefund = async(ctx, next) => {//申请退款
    ...
    try{
        ...
        await query(updateSql);
        let Progress = '雇主已申请退款'+Refund+'元，设计师将在3天内答复，逾期默认确认退款';
        let insertLogSql = `insert into log (Project_ID,Progress,Riqi) values (${parseInt(Project_ID)},'${Progress}','${moment().format("YYYY-MM-DD HH:mm:ss")}')`;
        await query(insertLogSql);
...
module.exports = {
    ...
    'POST /project/getNewsNumber': getNewsNumber
}