            ...
            if(data.type === 'publish'){
                ...
                if(employeeInfo && employeeInfo.length > 0){
                    if(employeeInfo[0].State === 0){
                        let addSql = `insert into project (Name,Desp,Wage,Wage_real,Length,Length_real,Employer_ID,Employee_ID,Riqi) values ('${data.Name}','${data.Desp}',${data.Wage},${data.Wage},${data.Length},${data.Length},'${data.Employer_ID}','${data.Employee_ID}','${moment().format("YYYY-MM-DD HH:mm:ss")}')`;
                        await query(addSql);
                        //增加设计师预约次数
